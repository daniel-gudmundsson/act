...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names, act_dim=4):
    ...
    def forward(self, obs, image, env_state, actions=None, is_pad=None):
        """
        obs: batch, obs_dim
        ...
        if is_training:
            ...
            # cls_joint_is_pad = torch.full((bs, 2), False).to(image.device) # False: not a padding
            ...
            encoder_output = self.encoder(encoder_input, src_key_padding_mask=is_pad)
            ...
            mu = logvar = None
            latent_sample = torch.zeros([self.latent_dim], dtype=torch.float32).to(image.device)
            latent_input = self.latent_out_proj(latent_sample)
        ...
        if self.backbones is not None:
            ...
            obs = self.input_proj_robot_state(obs)
            # env_state = self.input_proj_env_state(env_state)
            transformer_input = torch.stack([obs, latent_input], axis=0)
            hs = self.transformer(transformer_input, None, self.query_embed.weight, self.pos.weight)[0]
        a_hat = self.action_head(hs)
        is_pad_hat = self.is_pad_head(hs)

        # print('a_hat', a_hat)
        # print('is_pad_hat', is_pad_hat)
        # print('mu', mu)
        # print('logvar', logvar)
...
class CNNMLP(nn.Module):
    ...
    def forward(self, obs, image, env_state, actions=None):
        """
        obs: batch, obs_dim
        image: batch, num_cam, channel, height, width
        env_state: None
        ...
        features = torch.cat([flattened_features, obs], axis=1) # obs: 14
